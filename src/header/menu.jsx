...
const Menu = () => {
  return (
    <div className='rac-header-buttons'>
        {menu.map((item) => (
            <div className='rac-button-container' key={item.id}>
                {!item.button
                    ? <a href={item.href} className='rac-header-link'>{item.icon}</a>
                    : <button className='rac-header-link' onClick={item.onClick}>{item.icon}</button>
                }
